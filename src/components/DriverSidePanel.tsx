...
export default function DriverSidePanel({ driver, onClose, onUpdate }: { driver: any, onClose: () => void, onUpdate: () => void }) {
  const [metrics, setMetrics] = useState(driver.metrics || {
    trips_completed: '120',
    avg_rating: '4.8',
    hours_driven: '160',
    incidents: '0',
    fuel_efficiency: '92',
    next_license_renewal: '2025-10-15'
  });
  const [isEditingMetrics, setIsEditingMetrics] = useState(false);
  
  const [logs, setLogs] = useState<any[]>([]);
  const [loadingLogs, setLoadingLogs] = useState(true);
  const [newLog, setNewLog] = useState({ type: 'note', description: '' });
  ...
  const fetchLogs = async () => {
    setLoadingLogs(true);
    try {
      const res = await fetch(`/api/admin/usuarios/logs?driver_id=${driver.id}`);
  ...
  const handleSaveMetrics = async () => {
    try {
      const res = await fetch('/api/admin/usuarios/metrics', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: driver.id, metrics })
      });
      if (res.ok) {
  ...
  const handleAddLog = async () => {
    ...
    setSavingLog(true);
    try {
      const res = await fetch('/api/admin/usuarios/logs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driver_id: driver.id, ...newLog })
      });
      if (res.ok) {
        setNewLog({ type: 'note', description: '' });
        fetchLogs();
      }
    } catch (e) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full md:w-[500px] bg-zinc-950 border-l border-white/10 shadow-2xl z-[200] flex flex-col animate-in slide-in-from-right duration-300">
      <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/40">
        <div>
          <h2 className="text-2xl font-black text-white italic">{driver.full_name}</h2>
          <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest">{driver.email}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { key: 'trips_completed', label: 'Viajes Completados', type: 'number' },
              { key: 'avg_rating', label: 'Calificación', type: 'number' },
              { key: 'hours_driven', label: 'Horas (Mes)', type: 'number' },
              { key: 'incidents', label: 'Incidentes', type: 'number' },
              { key: 'fuel_efficiency', label: 'Eficiencia (%)', type: 'number' },
              { key: 'next_license_renewal', label: 'Venc. Licencia', type: 'date' },
            ].map((m) => (
              <div key={m.key} className="bg-zinc-900/50 p-4 rounded-2xl border border-white/5">
                ...
                {isEditingMetrics ? (
                  <input 
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-lg p-2 text-sm text-white focus:border-blue-500 outline-none"
                  />
                ) : (
          ...
          <div className="bg-zinc-900/30 p-4 rounded-2xl border border-white/5 mb-6 space-y-3">
            <div className="flex gap-2">
              ...
              >
                ...
                <option value="review">Evaluación</option>
              </select>
              <input 
                type="text" 
            ...
            {loadingLogs ? (
              ...
              logs.map((log: any) => (
                <div key={log.id} className="relative pl-6 before:absolute before:left-2 before:top-2 before:bottom-[-16px] before:w-[2px] before:bg-white/5 last:before:hidden">
                  <div className={`absolute left-[5px] top-2 w-[10px] h-[10px] rounded-full ring-4 ring-zinc-950 ${log.type === 'incident' ? 'bg-red-500' : log.type === 'review' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
                  <div className="bg-zinc-900/50 border border-white/5 p-3 rounded-xl">
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">
                        {log.type === 'incident' ? 'Incidente' : log.type === 'review' ? 'Evaluación' : 'Nota'}
                      </span>
